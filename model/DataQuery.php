...
class DataQuery {
	 ...
	 *
	 * Unlike {@see applyRelation} this will only work with non-polymorphic has_one relationships and will throw
	 * an error if a field cannot be found in the next has_one. This is the mechanism best used for applying
	 * complex sort conditions
	 *
	 * @param String|array $relation The array/dot-syntax relation to follow
	 * @return The model class of the related item
	 */
	public function applyLinearRelation($relation) {
		// NO-OP
		if(!$relation) {
			return $this->dataClass;
		}
		...
		foreach($relation as $rel) {
			$model = singleton($modelClass);
			if ($component = $model->hasOneComponent($rel)) {
				// Join only on has_one
				$this->joinHasOneRelation($modelClass, $rel, $component);
				$modelClass = $component;

			} elseif ($model->hasManyComponent($rel) || $model->manyManyComponent($rel)) {
				// Fail on non-linear relations
				throw new InvalidArgumentException(
						"$rel is not a linear relation on model $modelClass"
				);
			}
		}
