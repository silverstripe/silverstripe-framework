...
class DbDatetimeTest extends FunctionalTest {
	...
	function testDbDatetimeFormat() {
		if($this->supportDbDatetime) {
			$pageSegment = $this->objFromFixture("Page","first")->URLSegment;
			...
			$query = 'SELECT ' . $this->adapter->formattedDatetimeClause('"Created"', '%U') . ' AS test FROM "SiteTree" WHERE "URLSegment" = \''.$pageSegment.'\'';
			$result = DB::query($query)->value();

			$this->matchesRoughly($result, strtotime(DataObject::get_one('SiteTree',"\"URLSegment\" = '$pageSegment'")->Created), "SiteTree[$pageSegment]->Created as timestamp");
		}
	}
	...
	function testDbDatetimeDifference() {
		if($this->supportDbDatetime) {
			...
			$query = 'SELECT ' . $this->adapter->datetimeDifferenceClause('"LastEdited"', '"Created"') . ' AS "test" FROM "SiteTree" WHERE "URLSegment" = \''.$pageSegment.'\'';
			$result = DB::query($query)->value();
			$lastedited = Dataobject::get_one('SiteTree',"\"URLSegment\" = '$pageSegment'")->LastEdited;
			$created = Dataobject::get_one('SiteTree',"\"URLSegment\" = '$pageSegment'")->Created;
