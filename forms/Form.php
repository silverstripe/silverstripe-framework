...
class Form extends RequestHandler {
	...
	*/
	 ...
	 * @return Form
	 */
	public function loadDataFrom($data, $fieldMergeStrategy = 0, $fieldList = null) {
		...
		if ($fieldMergeStrategy === true) {
			$fieldMergeStrategy = self::MERGE_CLEAR_MISSING;
		}
		else if ($fieldMergeStrategy === false) {
			$fieldMergeStrategy = 0;
		}
		...
		if($dataFields) foreach($dataFields as $field) {
			...
			if($exists){
				if ($val != false || ($fieldMergeStrategy & self::MERGE_IGNORE_FALSEISH) != self::MERGE_IGNORE_FALSEISH){
					// pass original data as well so composite fields can act on the additional information
					$field->setValue($val, $data);
				}
			}
			else if(($fieldMergeStrategy & self::MERGE_CLEAR_MISSING) == self::MERGE_CLEAR_MISSING){
				$field->setValue($val, $data);
			}
		}
