...
/**
 * This class is is responsible for adding objects to another object's has_many and many_many
 * relation, as defined by the {@link RelationList} passed to the {@link GridField} constructor.
 *
 * Objects can be searched through an input field (partially matching one or more fields).
 *
 * Selecting from the results will add the object to the relation.
 *
 * Often used alongside {@link GridFieldDeleteAction} for detaching existing records from a
 * relationship.
 *
...
class GridFieldAddExistingAutocompleter implements GridField_HTMLProvider, GridField_ActionProvider, GridField_DataManipulator, GridField_URLHandler {
	 ...
	 *
	 * The fields support "dot-notation" for relationships, e.g. a entry called "Team.Name" will
	 * search through the names of a "Team" relationship.
	 *
	 * @example "array('Name', 'Email:StartsWith', 'Team.Name')"
	 *
	 * @var array
	 */
	...
	/**
	 * SSViewer template to render the results presentation.
	 *
	 * @var string
	 */
	...
	/**
	 * Text shown on the search field, instructing what to search for.
	 *
	 * @var string
	 */
	...
	public function getHTMLFragments($gridField) {
		...
		if($this->getSearchFields()) {
			...
			$searchFields = $this->scaffoldSearchFields($dataClass);
		}
	...
	public function handleAction(GridField $gridField, $actionName, $arguments, $data) {
		...
		if($actionName === 'find' && isset($data['autosuggest_search'])) {
	...
	/**
	 * Returns a json array of a search results.
	 *
	 * @param GridField $gridField
	...
	public function doSearch($gridField, $request) {
		...
		foreach($results as $result) {
			$json[$result->ID] = html_entity_decode(
				SSViewer::fromString($this->resultsFormat)->process($result)
			);
		}
	 ...
	 *
	 * @param SS_List $searchList
	 */
	public function setSearchList(SS_List $searchList) {
		...
		if($fieldSpecs = $obj->searchableFields()) {
			...
			foreach($fieldSpecs as $name => $spec) {
				if(is_array($spec) && array_key_exists('filter', $spec)) {
					// The searchableFields() spec defaults to PartialMatch, so we need to check
					// the original setting.

					// If the field is defined $searchable_fields = array('MyField'), then default
					// to StartsWith filter, which makes more sense in this context.
	...
	public function getPlaceholderText($dataClass) {
		...
		if($labels) {
			return _t(
				...
				'Find {type} by {name}',
				array(
					'type' => singleton($dataClass)->plural_name(),
					'name' => implode(', ', $labels),
				)
			);
		}
	...
	/**
	 * This will provide a StartsWith search that only returns a value if we are matching a single
	 * object.
	 *
	 * @param GridField $gridField
	 * @param string $field
	...
	protected function findSingleEntry($gridField, $field, $searchTerm, $dataClass) {
		...
		$searchResults = $fullList
			->subtract($existingList->limit(0))
			->filter($field . ':StartsWith', $searchTerm);
