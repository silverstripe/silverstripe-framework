...
class Requirements_Backend {
	 ...
	 */
    public function javascript($file, $attributes = null) {
	...
	function includeInHTML($templateFile, $content) {
		...
		if((strpos($content, '</head>') !== false || strpos($content, '</head ') !== false) && ($this->css || $this->javascript || $this->customCSS || $this->customScript || $this->customHeadTags)) {
            ...
            foreach(array_diff_key($this->javascript,$this->blocked) as $file => $params) {
                $path = $this->path_for_file($file);
                if($path) {
                    $attributes = (isset($params['attributes']) && !empty($params['attributes'])) ? " {$params['attributes']}" : "";
                    $jsRequirements .= "<script type=\"text/javascript\" src=\"$path\"{$attributes}></script>\n";
                }
            }
