  ...
  _jQuery2.default.entwine('ss', function ($) {
    $('.ss-gridfield').entwine({
      ...
      reload: function reload(ajaxOpts, successCallback) {
        ...
        var focusedElName = this.find(':input:focus').attr('name');
        ...
        $.ajax($.extend({}, {
          ...
          error: function error(e) {
            window.alert(_i18n2.default._t('GRIDFIELD.ERRORINTRANSACTION'));
            form.removeClass('loading');
          }
        }, ajaxOpts));
      },
    ...
    $('.ss-gridfield .col-buttons .action.gridfield-button-delete, .cms-edit-form .Actions button.action.action-delete').entwine({
      onclick: function onclick(e) {
        if (!window.confirm(_i18n2.default._t('TABLEFIELD.DELETECONFIRMMESSAGE'))) {
          e.preventDefault();
          return false;
        } else {
          this._super(e);
        ...
        this.autocomplete({
          source: function source(request, response) {
            ...
            $.ajax({
              ...
              error: function error(e) {
                window.alert(_i18n2.default._t('GRIDFIELD.ERRORINTRANSACTION', 'An error occured while fetching data from the server\n Please try again later.'));
              }
            });
          },
