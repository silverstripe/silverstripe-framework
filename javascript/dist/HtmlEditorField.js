  ...
  ss.editorWrappers.tinyMCE = function () {
    ...
    return {
      ...
      getConfig: function getConfig() {
        ...
        var config = (0, _jQuery2.default)(selector).data('config');
        var self = this;
      ...
      cleanLink: function cleanLink(href, node) {
        ...
        var cb = settings['urlconverter_callback'];
        if (cb) href = window[cb](href, node, true);
  ...
  _jQuery2.default.entwine('ss', function ($) {
    $('textarea.htmleditor').entwine({
      ...
      onadd: function onadd() {
        ...
        var ed = ss.editorWrappers[edClass]();
        this.setEditor(ed);
    ...
    $('form.htmleditorfield-linkform').entwine({
      ...
      getLinkAttributes: function getLinkAttributes() {
        ...
        var anchor = this.find(':input[name=Anchor]').val();
      ...
      resetFileField: function resetFileField() {
        ...
        var fileUpload = fileField.data('fileupload');
        var currentItem = fileField.find('.ss-uploadfield-item[data-fileid]');
      ...
      getAnchors: function getAnchors() {
        ...
        switch (linkType) {
          ...
          case 'internal':
            ...
            if (pageId) {
              $.ajax({
                url: $.path.addSearchParams(this.attr('action').replace('LinkForm', 'getanchors'), { 'PageID': parseInt(pageId, 10) }),
        ...
        if (data) {
          for (fieldName in data) {
            ...
            if (el.is(':checkbox')) {
              ...
              el = el.parents('.ss-uploadfield');(function attach(el, selected) {
                if (!el.getConfig()) {
                  setTimeout(function () {
                    attach(el, selected);
                  }, 50);
    ...
    $('form.htmleditorfield-mediaform').entwine({
      ...
      redraw: function redraw(updateExisting) {
        ...
        var node = this.getSelection();
        var hasItems = Boolean(this.find('.ss-htmleditorfield-file').length);
        var editingSelected = node.is('img');
        var insertingURL = this.hasClass('insertingURL');
        var header = this.find('.header-edit');
      ...
      showFileView: function showFileView(idOrUrl) {
        var self = this;
        var params = Number(idOrUrl) === idOrUrl ? { ID: idOrUrl } : { FileURL: idOrUrl };
    ...
    $('form.htmleditorfield-form.htmleditorfield-mediaform .add-url').entwine({
      ...
      onclick: function onclick(e) {
        ...
        var container = this.closest('.CompositeField');
        var form = this.closest('form');
    ...
    $('form.htmleditorfield-mediaform .ss-htmleditorfield-file').entwine({
      ...
      updateDimensions: function updateDimensions(constrainBy, maxW, maxH) {
        ...
        var heightEl = this.find(':input[name=Height]');
        var w = widthEl.val();
        var h = heightEl.val();
        var aspect;
    ...
    $('form.htmleditorfield-mediaform .ss-htmleditorfield-file.embed').entwine({
      ...
      getHTML: function getHTML() {
        ...
        var attrs = this.getAttributes();
        var extraData = this.getExtraData();
        var imgEl = $('<img />').attr(attrs).addClass('ss-htmleditorfield-file embed');
    ...
    $('form.htmleditorfield-mediaform .ss-uploadfield-item .ss-uploadfield-item-cancel').entwine({
      onclick: function onclick(e) {
        var form = this.closest('form');
        var file = this.closest('ss-uploadfield-item');
