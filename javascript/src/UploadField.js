...
$.widget('blueimpUIX.fileupload', $.blueimpUI.fileupload, {
  ...
  _onAdd: function (e, data) {
    ...
    if (data.files.length === 1 && data.files[0].error !== null) {
      ...
      $('.fileOverview .uploadStatus .state').text(i18n._t('AssetUploadField.UPLOADINPROGRESS', 'Please wait… upload in progress'))
  ...
  _onAlways: function (jqXHRorResult, textStatus, jqXHRorError, options) {
    ...
    if (typeof (jqXHRorError) === 'string') {
      ...
      $('.fileOverview .uploadStatus .state').text(i18n._t('AssetUploadField.FILEUPLOADCOMPLETED', 'File upload completed!'))
  ...
  attach: function (data) {
    ...
    var replaceFileID = data.replaceFileID
    var valid = true
...
$.entwine('ss', function ($) {
  $('div.ss-upload').entwine({
    ...
    onmatch: function () {
      ...
      var $dropZone = $('.ss-uploadfield-dropzone')
    ...
    openSelectDialog: function (uploadedFile) {
      ...
      dialog.find('iframe').bind('load', function (e) {
        ...
        var gridField = contents.find('.ss-gridfield')
    ...
    attachFiles: function (ids, uploadedFileId) {
      var self = this
      var config = this.getConfig()
      var indicator = $('<div class="loader" />')
      var target = (uploadedFileId) ? this.find(".ss-uploadfield-item[data-fileid='" + uploadedFileId + "']") : this.find('.ss-uploadfield-addfile')
  ...
  $('div.ss-upload .ss-uploadfield-item-remove:not(.ui-state-disabled), .ss-uploadfield-item-delete:not(.ui-state-disabled)').entwine({
    onclick: function (e) {
      ...
      var config = field.getConfig('changeDetection')
      var fileupload = field.data('fileupload')
      var item = this.closest('.ss-uploadfield-item')

      if (this.is('.ss-uploadfield-item-delete')) {
        if (window.confirm(i18n._t('UploadField.ConfirmDelete'))) {
  ...
  $('div.ss-upload:not(.disabled):not(.readonly) .ss-uploadfield-item-edit').entwine({
    onclick: function (e) {
      var self = this
      var editform = self.closest('.ss-uploadfield-item').find('.ss-uploadfield-item-editform')
  ...
  $('div.ss-upload .ss-uploadfield-item-editform').entwine({
    fitHeight: function () {
      ...
      // We set the height to match the form's outer height
      var bodyH = contents.find('form').outerHeight(true)
      // content's height + padding on iframe elem
      var iframeH = bodyH + (iframe.outerHeight(true) - iframe.height())
      // iframe height + padding on container elem
      var containerH = iframeH + (this.outerHeight(true) - this.height())
    ...
    toggleEditForm: function () {
      ...
      var status = itemInfo.find('.ss-uploadfield-item-status')

      var iframe = this.find('iframe').contents()
      var saved = iframe.find('#Form_EditForm_error')
