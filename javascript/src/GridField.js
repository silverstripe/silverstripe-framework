...
$.entwine('ss', function ($) {
  $('.ss-gridfield').entwine({
    ...
    reload: function (ajaxOpts, successCallback) {
      ...
      var focusedElName = this.find(':input:focus').attr('name') // Save focused element for restoring after refresh
      ...
      $.ajax($.extend({}, {
        ...
        error: function (e) {
          window.alert(i18n._t('GRIDFIELD.ERRORINTRANSACTION'))
          form.removeClass('loading')
        }
      }, ajaxOpts))
    },
  ...
  $('.ss-gridfield .action').entwine({
    onclick: function (e) {
      // filterstate should equal current state.
  ...
  $('.ss-gridfield .add-existing-autocompleter').entwine({
    ...
    toggleDisabled: function () {
      ...
      var inputHasValue = $input.val() !== ''
      var buttonDisabled = $button.is(':disabled')
  ...
  $('.ss-gridfield .col-buttons .action.gridfield-button-delete, .cms-edit-form .Actions button.action.action-delete').entwine({
    onclick: function (e) {
      if (!window.confirm(i18n._t('TABLEFIELD.DELETECONFIRMMESSAGE'))) {
        e.preventDefault()
        return false
      } else {
        this._super(e)
  ...
  $('.ss-gridfield .action.gridfield-button-print').entwine({
    ...
    onclick: function (e) {
      ...
      var form = this.closest('form')
      var data = form.find(':input.gridstate').serialize()
  ...
  $('.ss-gridfield .filter-header :input').entwine({
    onmatch: function () {
      ...
      var resetbtn = this.closest('.fieldgroup').find('.ss-gridfield-button-reset')
      ...
      this.autocomplete({
        source: function (request, response) {
          ...
          $.ajax({
            ...
            error: function (e) {
              window.alert(i18n._t('GRIDFIELD.ERRORINTRANSACTION', 'An error occured while fetching data from the server\n Please try again later.'))
            }
          })
        },
