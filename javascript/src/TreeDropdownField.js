...
$.entwine('ss', function ($) {
  ...
  $('.TreeDropdownField').entwine({
    ...
    openPanel: function () {
      ...
      var tree = this.find('.tree-holder')
    ...
    _riseUp: function () {
      var container = this
      var dropdown = this.getPanel()
      var toggle = this.find('.treedropdownfield-toggle-panel-link')
      var offsetTop = toggle.innerHeight()
      var elHeight
      var elPos
      var endOfWindow
    ...
    setTitle: function (title) {
      ...
      // separate view from storage (important for search cancellation)
    ...
    updateTitle: function () {
      var self = this
      var tree = self.find('.tree-holder')
      var val = this.getValue()
      var updateFn = function () {
        var val = self.getValue()
        if (val) {
          var node = tree.find('*[data-id="' + val + '"]')
          var title = node.children('a').find('span.jstree_pageicon') ? node.children('a').find('span.item').html() : null
    ...
    loadTree: function (params, callback) {
      var self = this
      var panel = this.getPanel()
      var treeHolder = $(panel).find('.tree-holder')
      var xhr
        ...
        success: function (html, status, xhr) {
          ...
          treeHolder
            ...
            .bind('loaded.jstree', function (e, data) {
              var val = self.getValue()
              var selectNode = treeHolder.find('*[data-id="' + val + '"]')
              var currentNode = data.inst.get_selected()
            ...
            .bind('select_node.jstree', function (e, data) {
              ...
              var id = $(node).data('id')
              if (!firstLoad && parseInt(self.getValue(), 10) === parseInt(id, 10)) {
