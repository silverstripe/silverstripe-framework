...
class PasswordExpirationMiddleware implements HTTPMiddleware
{
    ...
    /**
     * Session key for persisting Array of urls allowed to visit by users
     * with passwords expired (apart from the password change form)
     */
    const SESSION_KEY_WHITELIST = __CLASS__.'.whitelisted urls';
    ...
    /**
     * Enable the middleware to check password expiration
     *
     * @var bool
     *
     * @config
    ...
    /**
     * List of URLs allowed to visit by users with password expired
     * These URLs won't be redirected to the password change form.
     * Requests to these URLs will have the Member NOT deauthenticated
     * in case there is no password change form registered.
     *
     * @var array<string>
     *
     * @config
    ...
    /**
     * Whether we should allow the current request to be finished as is even if
     * the user password has been expired.
     *
     * WARNING: this is not a configurable attribute, but should be controlled by a LoginHandler
     *
     * @var bool
     */
    private static $allowCurrentRequest = false;
    ...
    protected function handleExpiredPassword(HTTPRequest $request): ?HTTPResponse
    {
        ...
        $configWhitelist = static::config()->get('whitelisted_urls');
        if (is_array($configWhitelist)) {
            $whitelist = array_merge($whitelist, $configWhitelist);
        }

        $sessionWhitelist = $session->get(static::SESSION_KEY_WHITELIST);
        if (is_array($sessionWhitelist)) {
            $whitelist = array_merge($whitelist, $sessionWhitelist);
        }
        ...
        if (in_array($currentUrl, $allowedUrls, true)) {
            return null;
        }
    ...
    /**
     * Allow a URL to be visited with expired password
     *
     * @param Session $session Session where we persist the whitelist
     * @param string $url URL to be whitelisted
     */
    public static function whitelistUrl(Session $session, $url)
    {
        $whitelist = $session->get(static::SESSION_KEY_WHITELIST);
        if (!is_array($whitelist)) {
            $whitelist = [];
        }
        $whitelist[] = $url;
        $session->set(static::SESSION_KEY_WHITELIST, $whitelist);
    }
