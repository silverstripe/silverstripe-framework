...
class SSViewer_Scope
{
     ...
     */
    public function scopeToIntermediateValue(string $name, array $arguments): static
    {
        $overlayIndex = false;
     ...
     */
    public function getOutputValue(string $name, array $arguments): string
    {
    ...
    public function getValueAsArgument(string $name, array $arguments): mixed
    {
        ...
        if ($this->hasOverlay($name)) {
            ...
            if ($on && isset($on->$name)) {
                $retval = $on->getRawDataValue($name, $arguments);
            }
    ...
    protected function getObj(string $name, array $arguments): ?ViewLayerData
    {
        if ($this->hasOverlay($name)) {
