...
class Deprecation {
	...
	/**
	 * Given a backtrace, get the module name from the most recent item in the trace
	 *
	 * @static
	 ...
	 * @return void
	 */
	public static function notice($string, $atVersion) {
		if(!Director::isDev()) return;

		$checkVersion = self::$version;

		// Getting a backtrace is slow, so only do it if we have module specific versions (otherwise no point)
		if(self::$module_version_overrides) {
			$module = self::get_calling_module_from_trace(debug_backtrace(0));
