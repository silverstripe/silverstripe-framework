...
class CKEditor
{
	...
	public function editor($name, $value = "", $config = array(), $events = array() )
	{
		...
		$out = "<textarea " . $attr . ">" . htmlspecialchars($value) . "</textarea>\n";
		if (!$this->initialized) {
			$out .= $this->init();
		}
	...
	*/
	...
	public function generateJS ($config = array(), $events = array()) {
		$_config = $this->configSettings($config, $events);

		$js = $this->returnGlobalEvents();
		$js .= "var ckEditorName = '".$this->textareaAttributes['id']."';";
		if (!empty($_config)) {
			$js .= "var ckEditorConfig = ".$this->jsEncode($_config).";";
		} else {
			$js .= "var ckEditorConfig = {};";
		}
		$js .= "CKEDITOR.plugins.addExternal('ssbuttons', 'plugins/ssbuttons/', 'plugin.js');";
		return $js;
	}
	/**
	...
	public function replace($id, $config = array(), $events = array())
	{
		...
		$js = $this->returnGlobalEvents();
		if (!empty($_config)) {
			$js .= "var CkEditorInstance = CKEDITOR.replace('".$id."', ".$this->jsEncode($_config).");";
		}
		else {
			$js .= "var CkEditorInstance = CKEDITOR.replace('".$id."');";
		}
		$out .= $this->script($js);
	...
	private function init()
	{
		...
		$out .= "<script type=\"text/javascript\" src=\"" . $ckeditorPath . 'config.js' . $args . "\"></script>\n";
