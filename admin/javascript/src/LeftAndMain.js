/* global History, jLayout, jQuery, DOMException, statusMessage */
...
$.entwine('ss', function ($) {
  ...
  $(window).on('message', function (e) {
    ...
    switch (data.type) {
      case 'event':
        ...
        break
      case 'callback':
        target[data.callback](data.data)
        break
    }
  })
  ...
  $(document).ajaxComplete(function (e, xhr, settings) {
    ...
    var reathenticate = xhr.getResponseHeader('X-Reauthenticate')
    var msgType = (xhr.status < 200 || xhr.status > 399) ? 'bad' : 'good'
    var ignoredMessages = ['OK']
  ...
  $('.cms-container').entwine({
    ...
    handleStateChange: function () {
      ...
      var fragments = state.data.pjax || 'Content'
      var headers = {}
      var fragmentsArr = fragments.split(',')
      var contentEls = this._findFragments(fragmentsArr)
    ...
    loadFragment: function (url, pjaxFragments) {
      ...
      var headers = {}
      var baseUrl = $('base').attr('href')
      var fragmentXHR = this.getFragmentXHR()
    ...
    handleAjaxResponse: function (data, status, xhr, state) {
      ...
      if (xhr.getResponseHeader('X-Reload') && xhr.getResponseHeader('X-ControllerURL')) {
        var baseUrl = $('base').attr('href')
        var rawURL = xhr.getResponseHeader('X-ControllerURL')
      ...
      try {
        ...
        $.each(newFragments, function (newFragment, html) {
          ...
          if (newContentEl.find('.cms-container').length) {
            throw new Error('Content loaded via ajax is not allowed to contain tags matching the ".cms-container" selector to avoid infinite loops')
          }
    ...
    _findFragments: function (fragments) {
      return $('[data-pjax-fragment]').filter(function () {
        ...
        var i
        var nodeFragments = $(this).data('pjaxFragment').split(' ')
        for (i in fragments) {
    ...
    saveTabState: function () {
      ...
      var selectedTabs = []
      var url = this._tabStateUrl()
      this.find('.cms-tabset,.ss-tabset').each(function (i, el) {
        var id = $(el).attr('id')

        // we need a unique reference
        if (!id) return
    ...
    restoreTabState: function (overrideStates) {
      ...
      var sessionData = hasSessionStorage ? window.sessionStorage.getItem('tabs-' + url) : null
      var sessionStates = sessionData ? JSON.parse(sessionData) : false
      ...
      this.find('.cms-tabset, .ss-tabset').each(function () {
        ...
        var tab
        var forcedTab = tabset.find('.ss-tabs-force-active')
    ...
    showLoginDialog: function () {
      ...
      var dialog = $('.leftandmain-logindialog')
      var url = 'CMSSecurity/login'
  ...
  $('.cms .cms-panel-link').entwine({
    onclick: function (e) {
      ...
      var url = (href && !href.match(/^#/)) ? href : this.data('href')
      var data = {pjax: this.data('pjaxTarget')}
  ...
  $('.cms .ss-ui-button-ajax').entwine({
    onclick: function (e) {
      ...
      var href = this.attr('href')
      var url = href ? href : this.data('href')
      ...
      jQuery.ajax({
        ...
        complete: function (xmlhttp, status) {
          ...
          try {
            if (typeof msg !== 'undefined' && msg !== null) eval(msg) // eslint-disable-line no-eval
          } catch (e) {}
  ...
  $('.cms .ss-ui-dialog-link').entwine({
    ...
    onclick: function () {
      ...
      var id = 'ss-ui-dialog-' + this.getUUID()
      var dialog = $('#' + id)
      if (!dialog.length) {
  ...
  $('.cms-search-form').entwine({
    onsubmit: function (e) {
      ...
      // always switch to the first tab (list view) when searching
  ...
  $('.cms-panel-deferred').entwine({
    ...
    redraw: function () {
      ...
      if (!url) throw new Error('Elements of class .cms-panel-deferred need a "data-url" attribute')
  ...
  $('.cms-tabset').entwine({
    ...
    redrawTabs: function () {
      ...
      var activeTab = this.find('ul:first .ui-tabs-active')
  ...
  $('#filters-button').entwine({
    onmatch: function () {
      ...
      // The current collapsed state of the element.
    ...
    showHide: function () {
      var self = this
      var $filters = $('.cms-content-filters').first()
...
window.statusMessage = function (text, type) {
