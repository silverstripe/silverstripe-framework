...
$.entwine('ss', function ($) {
  ...
  $('.cms-panel').entwine({
    ...
    onadd: function () {
      ...
      if (!this.find('.cms-panel-content').length) throw new Error('Content panel for ".cms-panel" not found')
    ...
    togglePanel: function (doExpand, silent, doSaveState) {
      ...
      // the content panel width always stays in "expanded state" to avoid floating elements
      this.width(newWidth)
