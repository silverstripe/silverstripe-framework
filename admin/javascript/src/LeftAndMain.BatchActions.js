...
$.entwine('ss.tree', function ($) {
  ...
  $('#Form_BatchActionsForm').entwine({
    ...
    refreshSelected: function (rootNode) {
      ...
      var ids = this.getIDs()
      var allIds = []
      var viewMode = $('.cms-content-batchactions-button')
      ...
      if (!actionUrl || parseInt(actionUrl, 10) === -1 || !viewMode.hasClass('active')) {
    ...
    onsubmit: function (e) {
      ...
      if (!ids || !ids.length) {
        window.alert(i18n._t('CMSMAIN.SELECTONEPAGE', 'Please select at least one page'))
        e.preventDefault()
        return false
      }
  ...
  $('.cms-content-batchactions-button').entwine({
    ...
    updateTree: function () {
      var tree = $('.cms-tree')
      var form = $('#Form_BatchActionsForm')
