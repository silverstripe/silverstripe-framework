...
$.entwine('ss.tree', function ($) {
  $('.cms-tree').entwine({
        ...
        .bind('move_node.jstree', function (e, data) {
          ...
          $.ajax({
            ...
            success: function () {
              ...
              if (parseInt($('.cms-edit-form :input[name=ID]').val(), 10) === parseInt(nodeID, 10)) {
        ...
        'crrm': {
          'move': {
            ...
            'check_move': function (data) {
              ...
              var isMovedOntoContainer = data.ot.get_container()[0] === data.np[0]
              var movedNodeClass = movedNode.getClassname()
              var newParentClass = newParent.getClassname()
              // Check allowedChildren of newParent or against root node rules
              var hints = self.getHints()
              var disallowedChildren = []
              var hintKey = newParentClass ? newParentClass : 'Root'
              var hint = (hints && typeof hints[hintKey] !== 'undefined') ? hints[hintKey] : null
    ...
    createNode: function (html, data, callback) {
      var self = this
      // Explicitly check for undefined as 0 is a valid ParentID
      var parentNode = data.ParentID !== void 0 ? self.getNodeByID(data.ParentID) : false
      var newNode = $(html)
