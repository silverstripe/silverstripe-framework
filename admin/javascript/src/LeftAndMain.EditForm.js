...
$.entwine('ss', function ($) {
  ...
  $('.cms-edit-form').entwine({
    ...
    confirmUnsavedChanges: function () {
      ...
      var confirmed = window.confirm(i18n._t('LeftAndMain.CONFIRMUNSAVED'))
      if (confirmed) {
    ...
    'from .htmleditor': {
      oneditorinit: function (e) {
        var self = this
        var field = $(e.target).closest('.field.htmleditor')
        var editor = field.find('textarea.htmleditor').getEditor().getInstance()
    ...
    restoreFieldFocus: function () {
      ...
      var sessionData = hasSessionStorage ? window.sessionStorage.getItem(this.attr('id')) : null
      var sessionStates = sessionData ? JSON.parse(sessionData) : false
      var elementID
      var tabbed = (this.find('.ss-tabset').length !== 0)
      var activeTab
      var elementTab
      var toggleComposite
      var scrollY
  ...
  $('.cms-edit-form .Actions input.action[type=submit], .cms-edit-form .Actions button.action').entwine({
    ...
    onclick: function (e) {
      ...
      if (
        ...
        !window.confirm(i18n._t('TABLEFIELD.DELETECONFIRMMESSAGE'))
      ) {
        e.preventDefault()
        return false
      }
