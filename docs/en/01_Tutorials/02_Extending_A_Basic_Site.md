...
Rather than using *$Date* directly, we use *$Date.Nice*. If we look in the [api:SilverStripe\ORM\FieldType\DBDate] documentation, we can see
...
Depending on your browser, you should see something like the picture below. If your browser doesn't support RSS, you will most likely see the XML output instead. For more on RSS, see [api:SilverStripe\Control\RSS\RSSFeed]
...
This template is very similar to the *ArticleHolder* template. The *ScaleWidth* method of the [api:SilverStripe\Assets\Image] class
