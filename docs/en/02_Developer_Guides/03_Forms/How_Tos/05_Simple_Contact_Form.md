...
We then create a [api:SilverStripe\Forms\FieldList] of the form actions, or the buttons that submit the form. Here we add a single form action, with the name 'submit', and the label 'Submit'. We'll use the name of the form action later.
...
The framework comes with a predefined validator called [api:SilverStripe\Forms\RequiredFields], which performs the common task of making sure particular fields are filled out. Below is the code to add validation to a contact form:
