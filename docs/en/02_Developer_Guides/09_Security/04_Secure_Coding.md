...
but there may be cases where you need to take care of escaping yourself. See [coding-conventions](/getting_started/coding_conventions)
and [datamodel](/developer_guides/model/data_model_and_orm) for ways to parameterise, cast, and convert your data.
...
To enable filtering, set the HtmlEditorField::$sanitise_server_side [configuration](/developer_guides/configuration) property to
true, e.g.
...
object-properties by [casting](/developer_guides/model/data_model_and_orm#casting) its string value into a `[api:DBField]` object.
...
You can force escaping on a casted value/object by using an [escape type](/developer_guides/model/data_model_and_orm) method in your template, e.g.
"XML" or "ATT". 
...
You can also use the built-in casting in PHP by using the *obj()* wrapper, see [datamodel](/developer_guides/model/data_model_and_orm)  .
...
or see the [Nginx](/getting_started/installation/how_tos/configure_nginx) installation documentation for details 
