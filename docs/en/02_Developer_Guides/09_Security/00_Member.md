...
The [api:SilverStripe\Security\Member] class comes with 2 static methods for getting information about the current logged in user.
...
This is the least desirable way of extending the [api:SilverStripe\Security\Member] class. It's better to use [api:SilverStripe\ORM\DataExtension]
(see below).
</div>

You can define subclasses of [api:SilverStripe\Security\Member] to add extra fields or functionality to the built-in membership system.
...
parent::getCMSFields() and manipulate the [api:SilverStripe\Forms\FieldList] from there.
...
Basic rule: Class [api:SilverStripe\Security\Member] should just be extended for entities who have some kind of login.
If you have different types of [api:SilverStripe\Security\Member]s in the system, you have to make sure that those with login-capabilities have
unique email-addresses (as this is used for login-credentials). 
For persons without login-capabilities (e.g. for an address-database), you shouldn't extend [api:SilverStripe\Security\Member] to avoid conflicts
with the Member-database. This enables us to have a different subclass of [api:SilverStripe\Security\Member] for an email-address with login-data,
...
A role extension is simply a subclass of [api:SilverStripe\ORM\DataExtension] that is designed to be used to add behaviour to [api:SilverStripe\Security\Member]. 
...
single tokens can be enforced by setting `[api:SilverStripe\Security\RememberLoginHash::$force_single_token] to true.
...
[api:SilverStripe\Security\Member]
