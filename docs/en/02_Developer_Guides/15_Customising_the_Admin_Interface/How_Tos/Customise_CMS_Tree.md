...
The tree is rendered through [api:SilverStripe\Admin\LeftAndMain::getSiteTreeFor()],
which recursively collects all nodes based on various filtering criteria.
The node strictly just has to implement the [api:SilverStripe\ORM\Hierarchy\Hierarchy] extension,
...
SiteTree is a [api:SilverStripe\ORM\DataObject] which is versioned by [api:SilverStripe\Versioned\Versioned] extension.
...
inherited subclass or [api:SilverStripe\ORM\DataExtension]. It is just really about how we change the return
