...
the [api:SilverStripe\Forms\Form] SilverStripe class, which is built solely around
...
inefficient to send back the HTML of entire tree. SilverStripe can serialize to and from JSON (see the [api:SilverStripe\Core\Convert] class), and jQuery deals very well with it through
