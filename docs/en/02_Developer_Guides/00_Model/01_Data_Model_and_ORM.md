...
All data tables in SilverStripe are defined as subclasses of [api:SilverStripe\ORM\DataObject]. The [api:DataObject] class represents a 
...
When rebuilding the database schema through the [api:SilverStripe\Core\Manifest\ClassLoader] the following additional properties are 
...
The `ORM` doesn't actually execute the [api:SilverStripe\ORM\Queries\SQLSelect] until you iterate on the result with a `foreach()` or `<% loop %>`.
...
See the [Lists](lists) documentation for more information on dealing with [api:SilverStripe\ORM\SS_List] instances.
...
To retrieve a news article, SilverStripe joins the [api:SiteTree], [api:SilverStripe\CMSModel\SiteTree\Page] and NewsPage tables by their ID fields. 
