...
`<class>_versions` table. To avoid this, use [api:SilverStripe\Versioned\Versioned::writeWithoutVersion()] instead.
...
multiple tables across an inherited table scheme (see [api:SilverStripe\Versioned\Versioned::augmentSQL()]). If possible, try to stick to 
...
* [api:SilverStripe\Versioned\Versioned]
