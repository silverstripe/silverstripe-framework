...
The shortcode handler for `File` has been moved to `SilverStripe\Assets\ViewSupport\FileShortcodeProvider` and the Image handler has been moved to `SilverStripe\Assets\ViewSupport\ImageShortcodeProvider`
...
* `handle_shortcode` methods are removed from `File` and `Image` and moved to their own classes in `SilverStripe\Assets\ViewSupport` and are named `FileShortcodeProvider` and `ImageShortcodeProvider` respectively.
...
* `Image::handle_shortcode` moved to `SilverStripe\Assets\ViewSupport\ImageShortcodeProvider::handle_shortcode`
