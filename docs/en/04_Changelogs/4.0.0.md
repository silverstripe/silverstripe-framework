...
This is the subset of all image types which may be assigned to the `[api:SilverStripe\Assets\Image]` dataobject, and may have
manipulations applied to it. This should be used as the file type restriction on any `[api:SilverStripe\AssetAdmin\Forms\UploadField]` which
...
In order to ensure you are using the correct table for any class a new [api:SilverStripe\ORM\DataObjectSchema] service
...
* Image manipulations have been moved into a new `[api:SilverStripe\Assets\ImageManipulation]` trait.
