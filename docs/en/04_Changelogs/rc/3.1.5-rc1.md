 ...
 * If errors could potentially be raised by any `[api:SilverStripe\Control\RequestHandler]` class such as a `[api:SilverStripe\Forms\Form]` or
   `[api:SilverStripe\Control\Controller]`, you may now add the new `[api:SilverStripe\ErrorPage\ErrorPageControllerExtension]` to this class to
