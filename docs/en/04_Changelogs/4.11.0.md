...
- Summary::DBText now splits sentences also on `?!:`, not only by `.`. Characters for splitting can be configured per `SilverStripe\ORM\FieldType\DBText:summarySentenceSeparators[]`
