...
__Note:__ GridField can only be used with datasets that are of the type `SS_List` such as `DataList`
 or `ArrayList`
</div>
...
A gridfield are often setup from a `Controller` that will output a form to the user. Even if there 
...
setting the display fields on the `GridFieldComponent`.
...
Gridfields behaviour and look is all depending of what config we're giving it. In the above example 
we did not specify one, so it picked a default config called `GridFieldConfig_Base`.

A config object is a container for `GridFieldComponents` that contains the actual functionality and
...
The fields displayed is from `DataObject::getCMSFields()`
...
Similar to `GridFieldConfig_RecordViewer` with the addition support of:
 ...
 - Viewing and changing a individual records data.
 - Deleting a record

	:::php
	$gridField = new GridField('pages', 'All pages', SiteTree::get(), GridFieldConfig_RecordEditor::create());

The fields displayed in the edit form is from `DataObject::getCMSFields()`
 
### GridFieldConfig_RelationEditor

Similar to `GridFieldConfig_RecordEditor`, but adds features to work on a records has-many or 
...
The `GridFieldComponent` are the actual workers in a gridfield. They can be responsible for:
...
Here is a list of components for generic use:
 ...
 - GridFieldToolbarHeader ([API](http://api.silverstripe.org/3.0/framework/gridfield/GridFieldToolbarHeader.html))
 - GridFieldSortableHeader ([API](http://api.silverstripe.org/3.0/framework/fields-relational/GridFieldSortableHeader.html))
 - GridFieldFilterHeader ([API](http://api.silverstripe.org/3.0/framework/fields-relational/GridFieldFilterHeader.html))
 - GridFieldDataColumns ([API](http://api.silverstripe.org/3.0/framework/fields-relational/GridFieldDataColumns.html))
 - GridFieldDeleteAction ([API](http://api.silverstripe.org/3.0/framework/gridfield/GridFieldDeleteAction.html))
 - GridFieldViewButton ([API](http://api.silverstripe.org/3.0/framework/GridFieldViewButton.html))
 - GridFieldEditButton ([API](http://api.silverstripe.org/3.0/framework/GridFieldEditButton.html))
 - GridFieldPaginator ([API](http://api.silverstripe.org/3.0/framework/fields-relational/GridFieldPaginator.html))
 - GridFieldDetailForm([API](http://api.silverstripe.org/3.0/framework/GridFieldDetailForm.html))

----

## Creating a custom GridFieldComponent

A single component are often using a number of interfaces.
...
GridField Components sets and gets data from the GridState.